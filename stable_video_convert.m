function stable_video_convert(init_video_path, output_video_path, varargin)
    ...
    for i = 1:length(files)
        ...
        command = sprintf(['ffmpeg -i %s -c:v libx264 -preset ultrafast -acodec copy %s'], init_video_file,output_video_file);
        % command = sprintf('ffmpeg -i %s -c:v copy -speed fastest %s', init_video_file, output_video_file);
        system(command);
    end
end